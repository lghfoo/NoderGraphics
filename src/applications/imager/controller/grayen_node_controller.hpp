...
namespace Imager {
    ...
    public:
        GrayenNodeController(PObject graphics_arg = nullptr,
            ...
            // todo: use shared_ptr?
            input_port_controller = new PortController(value_node->GetInputPort(), value_graphics->GetUI<PortProxy>(GrayenNodeGraphics::INPUT_PORT));
            output_port_controller = new PortController(value_node->GetOutputPort(), value_graphics->GetUI<PortProxy>(GrayenNodeGraphics::OUTPUT_PORT));
        }
    };
}
